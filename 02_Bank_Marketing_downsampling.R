...
num_rows_positive <- sum(bank$y == "term.deposit")

positive <- bank %>% filter(y == "term.deposit")
negative <- bank %>% filter(y == "no")

indices <- sample(nrow(negative), size = num_rows_positive)
bank <- rbind( negative[indices,] , positive)
